...
if __name__ == "__main__":
    ...
    json_dict[
        "description"
    ] = "Multi-Atlas Labeling Beyond the Cranial Vault Abdominal Organ Segmentation"
    ...
    json_dict["training"] = [
        {
            "image": "./imagesTr/%s" % train_patient_name,
            "label": "./labelsTr/%s" % train_patient_name,
        }
        for i, train_patient_name in enumerate(train_patient_names)
    ]
    json_dict["test"] = [
        "./imagesTs/%s" % test_patient_name for test_patient_name in test_patient_names
    ]
