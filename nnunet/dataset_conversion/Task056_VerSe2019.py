...
def check_if_all_in_good_orientation(
    ...
    for n in filenames:
        img = sitk.GetArrayFromImage(
            sitk.ReadImage(join(imagesTr_folder, n[:-7] + "_0000.nii.gz"))
        )
...
def evaluate_verse_folder(folder_pred, folder_gt, out_json="/home/fabian/verse.json"):
    ...
    assert all(
        [isfile(join(folder_pred, i)) for i in files_gt_bare]
...
def print_unique_labels_and_their_volumes(
    ...
    for k in volumes.keys():
        v = volumes[k]
        if (
            print_only_if_vol_smaller_than is not None
            and v > print_only_if_vol_smaller_than
        ):
            pass
        else:
...
if __name__ == "__main__":
    ...
    test_files_base = [
        i[: -len(".nii.gz")]
        for i in subfiles(join(base, "test"), join=False, suffix=".nii.gz")
    ]
    ...
    train_patient_names = [
        i[: -len("_seg.nii.gz")]
        for i in subfiles(join(base, "train"), join=False, suffix="_seg.nii.gz")
    ]
    ...
    test_patient_names = [
        i[:-7] for i in subfiles(join(base, "test"), join=False, suffix=".nii.gz")
    ]
    ...
    _ = [
        print_unique_labels_and_their_volumes(i, 1000)
        for i in subfiles(labelstr, suffix=".nii.gz")
    ]
