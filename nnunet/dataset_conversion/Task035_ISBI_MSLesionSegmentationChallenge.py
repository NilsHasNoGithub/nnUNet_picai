...
if __name__ == "__main__":
    # convert to nifti.gz
    dirs = [
        "/media/fabian/My Book/MedicalDecathlon/Task035_ISBILesionSegmentation/imagesTr",
        ...
        for patientid in range(1, 15):
            ...
            for t in range(1, 10):
                patient_files = [
                    i
                    for i in all_files
                    if i.find("%02.0d_%02.0d_" % (patientid, t)) != -1
                ]
                if not len(patient_files) == 4:
                    continue

                flair_file = [
                    i for i in patient_files if i.endswith("_flair_pp.nii.gz")
                ][0]
                mprage_file = [
                    i for i in patient_files if i.endswith("_mprage_pp.nii.gz")
                ][0]
                ...
                os.rename(
                    join(folder, flair_file),
                    join(folder, "case__%02.0d__%02.0d_0000.nii.gz" % (patientid, t)),
                )
                os.rename(
                    join(folder, mprage_file),
                    join(folder, "case__%02.0d__%02.0d_0001.nii.gz" % (patientid, t)),
                )
                os.rename(
                    join(folder, pd_file),
                    join(folder, "case__%02.0d__%02.0d_0002.nii.gz" % (patientid, t)),
                )
                os.rename(
                    join(folder, t2_file),
                    join(folder, "case__%02.0d__%02.0d_0003.nii.gz" % (patientid, t)),
                )
    ...
    for patientid in range(1, 6):
        for t in range(1, 6):
            fnames_original = subfiles(
                train_folder,
                prefix="case__%02.0d__%02.0d" % (patientid, t),
                suffix=".nii.gz",
                sort=True,
            )
            ...
            for mask in [1, 2]:
                ...
                if isfile(f):
                    os.rename(
                        ...
                        join(
                            labels_folder,
                            "case__%02.0d__%02.0d__mask%d.nii.gz"
                            % (patientid, t, mask),
                        ),
                    )
        ...
        for t in range(1, 6):
            for mask in [1, 2]:
                if isfile(
                    join(
                        labels_folder,
                        "case__%02.0d__%02.0d__mask%d.nii.gz" % (patientid, t, mask),
                    )
                ):
                    tr_files.append(
                        "case__%02.0d__%02.0d__mask%d.nii.gz" % (patientid, t, mask)
    ...
    json_dict["modality"] = {"0": "flair", "1": "mprage", "2": "pd", "3": "t2"}
    ...
    json_dict["training"] = [
        {
            "image": "./imagesTr/%s.nii.gz" % i[:-7],
            "label": "./labelsTr/%s.nii.gz" % i[:-7],
        }
        for i in tr_files
    ]
    ...
    case_identifiers = np.unique(
        [
            i[:-12]
            for i in subfiles(
                "/media/fabian/My Book/MedicalDecathlon/MedicalDecathlon_raw_splitted/Task035_ISBILesionSegmentation/imagesTr",
                suffix=".nii.gz",
                join=False,
            )
        ]
    ...
    for f in range(5):
        ...
        splits[-1]["val"] = np.array(
            [i for i in case_identifiers if i.startswith("case__%02d__" % (f + 1))]
        )
    ...
    save_pickle(
        splits,
        join("/media/fabian/nnunet/Task035_ISBILesionSegmentation", "splits_final.pkl"),
    )
