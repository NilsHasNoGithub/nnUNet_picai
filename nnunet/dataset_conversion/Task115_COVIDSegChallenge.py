...
def postprocess_submission(
    folder_ct, folder_pred, folder_postprocessed, bbox_distance_to_seg_in_cm=7.5
):
    """
    ...
    for i in ids:
        ...
        sitk.WriteImage(
            seg_corr, join(folder_postprocessed, "volume-covid19-A-" + i + "_ct.nii.gz")
        )
...
def manually_set_configurations():
    ...
    fullres_stage["pool_op_kernel_sizes"] = [
        [1, 2, 2],
        [1, 2, 2],
        [2, 2, 2],
        [2, 2, 2],
    ...
    fullres_stage["conv_kernel_sizes"] = [
        [1, 3, 3],
        [1, 3, 3],
        [3, 3, 3],
        [3, 3, 3],
    ...
    increase_batch_size(
        join(task115_dir, "nnUNetPlansv2.1_plans_3D.pkl"),
        join(task115_dir, "nnUNetPlansv2.1_bs3x_plans_3D.pkl"),
        3,
    )
    increase_batch_size(
        join(task115_dir, "nnUNetPlansv2.1_plans_3D.pkl"),
        join(task115_dir, "nnUNetPlansv2.1_bs5x_plans_3D.pkl"),
        5,
    )
    ...
    """
    plans = load_pickle(
        join(task115_dir, "nnUNetPlans_FabiansResUNet_v2.1_plans_3D.pkl")
    )
    fullres_stage = plans["plans_per_stage"][1]
    ...
    save_pickle(
        plans, join(task115_dir, "nnUNetPlans_FabiansResUNet_v2.1_custom_plans_3D.pkl")
    )
...
def check_same(img1: str, img2: str):
    ...
    if not np.all([i == j for i, j in zip(img1.shape, img2.shape)]):
        ...
        if same:
            ...
            print(
                diffs[
                    np.random.choice(len(diffs), min(3, diffs.shape[0]), replace=False)
                ]
            )
            return False


def check_dataset_same(
    dataset_old="/home/fabian/Downloads/COVID-19-20/Train",
...
if __name__ == "__main__":
    ...
    downloaded_data_dir = (
        "/home/fabian/data/COVID-19-20_officialCorrected/COVID-19-20_v2/"
    )
