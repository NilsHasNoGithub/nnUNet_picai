...
def apply_threshold_to_folder(
    ...
    p = Pool(processes)
    p.starmap(
        apply_brats_threshold,
        zip(
            niftis,
            [folder_out] * len(niftis),
            [threshold] * len(niftis),
            [replace_with] * len(niftis),
        ),
    )
...
def determine_brats_postprocessing(
    folder_with_preds,
    folder_with_gt,
    postprocessed_output_dir,
    processes=8,
    thresholds=(0, 10, 50, 100, 200, 500, 750, 1000, 1500, 2500, 10000),
    replace_with=2,
):
    # find pairs
    ...
    results = p.starmap_async(
        load_niftis_threshold_compute_dice,
    ...
    p.starmap(
        ...
        zip(
            nifti_pred,
            [postprocessed_output_dir] * len(nifti_pred),
            [best_threshold] * len(nifti_pred),
    ...
    save_pickle(
        (thresholds, means, best_threshold, all_dc_per_threshold),
...
def collect_and_prepare(base_dir, num_processes=12, clean=False):
    ...
    for e in experiments:
        ...
        try:
            ...
            if clean or not isfile(join(o_p, "threshold.pkl")):
                determine_brats_postprocessing(
                    o,
                    gt_dir,
                    o_p,
                    num_processes,
                    thresholds=list(np.arange(0, 760, 10)),
    ...
    with open(join(base_dir, "cv_summary.csv"), "w") as f:
        ...
        for e in successful:
            ...
            if isfile(expected_nopp):
                res = np.loadtxt(expected_nopp, dtype=str, skiprows=0, delimiter=",")[
                    -2
                ]
        ...
        for folder in ["cv_results", "cv_results_pp"]:
            for ex in subdirs(join(base_dir, folder), join=False):
                ...
                if clean or not isfile(expected):
                    evaluate_regions(
                        join(base_dir, folder, ex), gt_dir, regions, num_processes
                    )
                if isfile(expected):
    ...
    for e in successful:
        if isdir(join(base_dir, "predVal", e)):
            ...
            if len(files) != expected_num_cases:
                print(
                    e,
                    "prediction not done, found %d files, expected %s"
                    % (len(files), expected_num_cases),
                )
                continue
            ...
            if threshold > 1000:
                threshold = 750  # don't make it too big!
            apply_threshold_to_folder(
                currdir, output_folder, threshold, replace_with, num_processes
            )
    ...
    currdir = join(
        base_dir,
        "predVal",
        "nnUNetTrainerV2BraTSRegions_DA3_BN__nnUNetPlansv2.1_bs5_15fold",
    )
    output_folder = join(
        base_dir,
        "predVal_PP",
    ...
    for source in ["predVal", "predVal_PP"]:
        for e in has_val_pred + [
            ...
            convert_labels_back_to_BraTS_2018_2019_convention(
                expected_source_folder, target_folder
            )
...
def convert_all_to_BraTS(input_folder, output_folder, expected_num_cases=125):
    for s in subdirs(input_folder, join=False):
        ...
        if len(nii) != expected_num_cases:
            ...
            convert_labels_back_to_BraTS_2018_2019_convention(
                join(input_folder, s), target_dir, num_processes=6
            )
...
def evaluate_BraTS_folder(
    ...
    assert all(
        [i in nii_gt for i in nii_pred]
    ), "not all predicted niftis have a reference file!"
    if strict:
        assert all(
            [i in nii_pred for i in nii_gt]
        ), "not all gt niftis have a predicted file!"
...
def score_and_postprocess_model_based_on_rank_then_aggregate():
    ...
    tmp = np.loadtxt(
        join(out, experiments_with_full_cv[0], "results.csv"),
        dtype="str",
        delimiter=",",
    )
    num_cases = len(tmp) - 1
    ...
    for e in experiments_with_full_cv:
        ...
        for t in thresholds:
            ...
            if not isfile(join(output_directory, "results.csv")):
                apply_threshold_to_folder(
                    input_folder, output_directory, t, replace_with, processes=16
                )
    ...
    for e in experiments_with_full_cv:
        ...
        pp_models = [
            j
            for j, i in enumerate(experiment_names)
            if i.split("___")[0] == e and i != e
        ]
        if len(pp_models) > 0:
            ...
            if not isdir(predicted_val):
                ...
                if len(files) != expected_num_cases_val:
                    print(
                        e,
                        "has missing val cases. found: %d expected: %d"
                        % (len(files), expected_num_cases_val),
                    )
                else:
                    apply_threshold_to_folder(
                        predicted_val,
                        join(pred_val_base, e),
                        best_threshold,
                        replace_with,
                        num_processes,
                    )
    ...
    e = "nnUNetTrainerV2BraTSRegions_DA3_BN__nnUNetPlansv2.1_bs5"
    pp_models = [
        j for j, i in enumerate(experiment_names) if i.split("___")[0] == e and i != e
    ]
    ranks = [final_ranks[i] for i in pp_models]
    best_idx = np.argmin(ranks)
    best = experiment_names[pp_models[best_idx]]
...
if __name__ == "__main__":
    ...
    for p in subdirs(cur, join=False):
        ...
        assert all([isfile(t1), isfile(t1c), isfile(t2), isfile(flair), isfile(seg)]), (
            "%s" % patient_name
        )
