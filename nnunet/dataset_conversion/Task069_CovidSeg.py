...
if __name__ == "__main__":
    ...
    training_data = sitk.GetArrayFromImage(
        sitk.ReadImage(join(download_dir, "tr_im.nii.gz"))
    )
    training_labels = sitk.GetArrayFromImage(
        sitk.ReadImage(join(download_dir, "tr_mask.nii.gz"))
    )
    ...
    for f in range(5):
        ...
        sitk.WriteImage(
            sitk.GetImageFromArray(data), join(imagestr, this_name + "_0000.nii.gz")
        )
        sitk.WriteImage(
            sitk.GetImageFromArray(labels), join(labelstr, this_name + ".nii.gz")
        )
