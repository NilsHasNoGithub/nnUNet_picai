...
if __name__ == "__main__":
    base = (
        "/media/fabian/My Book/datasets/CellTrackingChallenge/Fluo-C3DH-A549_ManAndSim"
    )
    task_id = 75
    ...
    splits.append(
        {
            "train": [
                i[:-7] for i in cases if i.startswith("01_") or i.startswith("02_SIM")
            ],
            "val": [
                i[:-7]
                for i in cases
                if i.startswith("02_") and not i.startswith("02_SIM")
            ],
        }
    )
    splits.append(
        {
            "train": [
                i[:-7] for i in cases if i.startswith("02_") or i.startswith("01_SIM")
            ],
            "val": [
                i[:-7]
                for i in cases
                if i.startswith("01_") and not i.startswith("01_SIM")
            ],
        }
    )
    splits.append(
